...
func TestPostMealplanItemShoppingListSucc(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/familys/1/mealplan/1/shopping-list/recipeingredients/1", nil)
	r.ServeHTTP(w, req)
	assert.Equal(t, 200, w.Code)
...
func TestPostMealplanItemShoppingListBadReq(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("POST", "/familys/1000/mealplan/1000/shopping-list/recipeingredients/1000", nil)
	r.ServeHTTP(w, req)
